"""
AI Knowledge Assistant - CLI Chatbot.
Run from project root: python app.py

Features:
- Paste document
- Generate summary (LangChain + HuggingFace)
...
# Default HuggingFace model (small, runs on CPU)
...
def main():
    ...
    # Lazy-load LLM chains (slow to load model)
    ...
    while True:
        ...
        if cmd == "document":
            rest = user_input[9:].strip()  # after "document "
            if rest:
                # Load from file: document path/to/file.txt
                filepath = Path(rest)
                if filepath.exists():
                    try:
            ...
            while True:
                try:
                    ...
                    break
                # If first input has newlines, whole paste was sent at once — use it and stop
                if first and "\n" in line:
        ...
        if cmd == "ask":
            if not current_document:
                print("Assistant: Paste a document first (command: document).")
                continue
            # Next line is the question (or use rest of input)
        ...
        if cmd == "sentiment":
            # Analyze current document or the sentence user typed after "sentiment"
        ...
        # Unknown: treat as a general question if we have a document
        if current_document:
